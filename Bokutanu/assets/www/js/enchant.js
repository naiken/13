 ...
 * @type {Object}
 * @namespace
 * {@link enchant.Tween} 繧ｯ繝ｩ繧ｹ縺ｧ逕ｨ縺�ｋ繧､繝ｼ繧ｸ繝ｳ繧ｰ髢｢謨ｰ縺ｮ繝ｩ繧､繝悶Λ繝ｪ蜷榊燕遨ｺ髢.
 */
enchant.Easing = {
    /**
     * @param t
...
enchant.Timeline = enchant.Class.create(enchant.EventTarget, {
     ...
     *
     ﾂﾂﾂﾂ * Reading a tl.enchant.js, all classes (Group, Scene, Entity, Label, Sprite) of the Node class that inherits
     ﾂﾂﾂﾂ * Tlthe property, an instance of the Timeline class is generated.
     ﾂﾂﾂﾂ * Time-line class has a method to add a variety of actions to himself,
     ﾂﾂﾂﾂ * entities can be animated and various operations by using these briefly.
     ﾂﾂﾂﾂ * You can choose time based and frame based(default) animation.
     *
...
enchant.Action = enchant.Class.create(enchant.ActionEventTarget, {
     ...
     * @class
     * Action class.
ﾂﾂﾂﾂﾂ* Actions are units that make up the time line,
ﾂﾂﾂﾂ * It is a unit used to specify the action you want to perform.
ﾂﾂﾂﾂ * Action has been added to the time line is performed in order.
ﾂﾂﾂﾂ *
ﾂﾂﾂﾂﾂ* Actionstart, actiontick event is fired when the action is started and stopped,
ﾂﾂﾂﾂ * When one frame has elapsed actiontick event is also issued.
ﾂﾂﾂﾂ * Specify the action you want to perform as a listener for these events.
 ﾂﾂﾂ * The transition to the next action automatically the number of frames that are specified in the time has elapsed.
     *
     * @constructs
     * @param param
